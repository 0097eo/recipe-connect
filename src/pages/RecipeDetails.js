...
const RecipeDetails = () => {
  ...
  useEffect(() => {
    const fetchRecipeDetails = async () => {
      ...
      try {
        setLoading(true);
        const response = await axios.get(`https://recipeconnect-e0mw.onrender.com/recipes/${id}`);
  ...
  const handleBookmark = async () => {
    ...
    try {
      if (isBookmarked) {
        await axios.delete(`https://recipeconnect-e0mw.onrender.com/recipes/${id}/bookmarks`, {
          headers: { 'Authorization': `Bearer ${localStorage.getItem('access_token')}` }
        });
      } else {
        await axios.post(`https://recipeconnect-e0mw.onrender.com/recipes/${id}/bookmarks`, {}, {
          headers: { 'Authorization': `Bearer ${localStorage.getItem('access_token')}` }
        });
      }
  ...
  const handleRating = async (value) => {
    ...
    try {
      await axios.post(`https://recipeconnect-e0mw.onrender.com/recipes/${id}/ratings`, { value }, {
        headers: { 'Authorization': `Bearer ${localStorage.getItem('access_token')}` }
      });
      setUserRating(value);
      const response = await axios.get(`https://recipeconnect-e0mw.onrender.com/recipes/${id}`);
  ...
  const handleCommentSubmit = async (e) => {
    ...
    try {
      await axios.post(`https://recipeconnect-e0mw.onrender.com/recipes/${id}/comments`, { content: comment }, {
        headers: { 'Authorization': `Bearer ${localStorage.getItem('access_token')}` }
      });
      setComment('');
      // Reload comments
